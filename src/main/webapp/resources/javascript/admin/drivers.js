$clickUpdate = function(id) {
	...
	data["nationality"] = $("#nationality_" + id).val();
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#id_" + id).html(data.driver.id);
			alert("Successfully saved driver " + data.driver.name + " with ID " + data.driver.id);
		},
		error: function (e) {
			button.disabled = false;
			alert("Failed to save driver " + $("#name_" + id).val());
		}
	});
}
...
jQuery(document).ready(function() {
	$("#add").click(function(event) {
		...
		element3.type = "text";
		element3.id = "number_" + (rowCount - 1);
		cell3.appendChild(element3);

		var cell4 = row.insertCell(3);
		var element4 = document.createElement("input");
		element4.type = "text";
		element4.id = "avatar_" + (rowCount - 1);
		cell4.appendChild(element4);
		
		var cell5 = row.insertCell(4);
		var element5 = document.createElement("input");
		element5.type = "text";
		element5.id = "nationality_" + (rowCount - 1);
